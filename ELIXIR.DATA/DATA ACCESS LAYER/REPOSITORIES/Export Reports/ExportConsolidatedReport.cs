        ...
        public async Task<Unit> Handle(ExportConsolidatedReportQuery request, CancellationToken cancellationToken)
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                var headers = new List<string>
                {
                    ...
                    "Item Description", "Uom", "Quantity", "Unit Price", "Amount", "Transaction Type",
                ...
                for (var index = 1; index <= orders.Count; index++)
                {
                    ...
                    row.Cell(11).Value = orders[index - 1].Source;
                    row.Cell(12).Value = orders[index - 1].Reason;
                    row.Cell(13).Value = orders[index - 1].Reference;
                    row.Cell(14).Value = orders[index - 1].Encoded;
                    row.Cell(15).Value = orders[index - 1].CompanyCode;
                    row.Cell(16).Value = orders[index - 1].CompanyName;
                    row.Cell(17).Value = orders[index - 1].DepartmentCode;
                    row.Cell(18).Value = orders[index - 1].DepartmentName;
                    row.Cell(19).Value = orders[index - 1].LocationCode;
                    row.Cell(20).Value = orders[index - 1].LocationName;
                    row.Cell(21).Value = orders[index - 1].AccountTitleCode;
                    row.Cell(22).Value = orders[index - 1].AccountTitle;
                    row.Cell(23).Value = orders[index - 1].Category;
                }
